...
[Вес](#вес)  
[Pod Topology Spread Constaints](#pod-topology-spread-constaints)  
...
```sh
kubectl -n work scale --replicas=3 depoloyment testdp
```  
У нас все так же останеться 3 пода, но при этом 3 под будет в состоянии Pending, тк ```podAntiAffinity``` запрещает размещать на ноде более одного пода со списком меток ```app.kubernetes.io/name: testdp``` и ```app.kubernetes.io/version: v0.0.1```  
