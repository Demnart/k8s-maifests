...
Начнём писать манифест Deployment:
...
```shell
kubectl apply -f manifests/03-deployment.yml
```
...
Deployment берет на себя ответственность за перезапуск подов.
...
```shell
kubectl apply -f deployment-newcontainer.yml
```
